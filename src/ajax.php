...
if (isset($_GET['q'])) {
    ...
    $insertar = mysqli_query($conexion, "INSERT INTO facturacion(id_cliente, total, id_usuario) VALUES ($id_cliente, '$total', $id_user)");
    if ($insertar) {
        $id_maximo = mysqli_query($conexion, "SELECT MAX(id) AS total FROM facturacion");
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $total = $row['total'];
            $insertarDet = mysqli_query($conexion, "INSERT INTO detalle_facturacion (id_producto, id_venta, cantidad, precio, total) VALUES ($id_producto, $ultimoId, $cantidad, '$precio', '$total')");
